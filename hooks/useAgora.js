...
const useAgora = (client) => {
  ...
  //   }
    ...
    const joinLocalVideoTrack = async() => {
      try {
        if (localVideoTrack) {
          ...
          const cameraTrack = await AgoraRTC.createCameraVideoTrack({ encoderConfig: '720p_2' })
     ...
     */
    const join = async(appid, channel, token=null, uid=null, options, callBacks)=>{
      console.log('options', options)
        ...
        try {
            ...
            if(options.video){
              cameraTrack = await AgoraRTC.createCameraVideoTrack({ encoderConfig: '720p_2' });
              setLocalVideoTrack(cameraTrack);
            ...
            window.audioTrack = microphoneTrack;
            setJoinState(true);
        } catch (error) {
    ...
    const changeVideoDevice = async (deviceId = "") => {
      try {
        ...
        console.error(e, "While switching camera");
      }
    };
