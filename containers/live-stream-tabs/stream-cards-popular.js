...
const PopularStreamCards = (props) => {
  ...
  React.useEffect(() => {
    if (!popularStream.data.length) {
      ...
      console.log("DATA ==>>", popularStream.data);
      setIsLoading(false);
    }
  }, []);
    ...
    <>
      <PullToRefresh onRefresh={handleRefresh} fetchMoreThreshold={500}>
        <div className={mobileView ? 'col-12' : 'col-12 px-0 pt-4'}>
          <PaginationIndicator
            id="popular__stream__cards"
            totalData={popularStream.data || []}
            totalCount={popularStream.totalCount}
          ...
          >
            {(!isLoading) ? popularStream.data.length ? (
              popularStream.data.map((stream) => (
                <div className={mobileView ? "col-6 mb-2" : "col-auto  mb-3 ml-3"} key={stream.streamId}>
                  <LiveStreamCards mobileView={mobileView} streamData={stream} currentTimeStamp={currentTimeStamp} setWarnStateIssue={setWarnStateIssue} callStreamFunction={callStreamFunction} />
                </div>
              ))
            ) : (
              <div
...
export default PopularStreamCards;
