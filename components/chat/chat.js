...
import { close_drawer, open_dialog, open_drawer, Toast } from "../../lib/global";
...
class Chat extends React.Component {
	scrollHeight = 0;
	vipChat = JSON.parse(getLocalStorage("vipChat") || "{}") || {};
	...
	constructor(props) {
		...
		this.state = {
			...
			loadMore: false,
			chatLoading: false,
			vipChatCount: this.vipChat?.chatCount || props.chatData.vipMessage,
	...
	onMessageTyping = () => {
		let userId =
			this.state.chatData.senderId == this.props.userData._id
				? this.state.chatData.receiverId
				: this.state.chatData.senderId;
		let secretId = this.state.chatData && this.state.chatData.chatId; //chatId
		sendTypeAck.next({
	...
	handleUnmountHeader = () => {
		this.props.actionDispatch(setActiveChat("123"));
	};
	async componentDidMount() {
				...
				this.setState(
					...
					() => {
						...
						setTimeout(() => {
							...
							// !this.props.chat.userList[userId] &&
		...
		//   }
		// );
		// console.log('query params::', this.props.query)

		// if (this.props.query?.p) {
		// 	const vipChat = JSON.parse(getLocalStorage("vipChat") || "{}") || {};

		// 	if (!this.state.vipChatCount && vipChat?.chatCount) {
		// 		console.log("vipChat", vipChat, vipChat.chatCount);
		// 		this.setState({ vipChatCount: 10 }, () => {
		// 			console.log("vipChat - chat updated", this.state.vipChatCount);
		// 		});
		// 	}
		// }
	}
	...
	componentWillUnmount() {
		...
		try {
			...
			console.error("rx unsubscribe error", userStatus, e);
		}
	...
	//     this.deleteChatDialog(this.deleteChatList, this.props.close_dialog)
	//   );
	// };

	deleteDialog = () => {
		open_drawer("DELETE_CHAT",
			this.deleteChatDialog(this.deleteChatList, close_drawer),
			"bottom"
		);
	};

	blockUser = () => {
		this.props.open_dialog("confirmDialog", {
	...
	deleteChatList = async () => {
		...
		// console.log("delete chat list", chatId);
		try {
			...
			if (e.response) {
				...
				// Toast(e.response.data.message, "success");
			}
			close_drawer();
	...
	sendMessage = (message, type = 1, extraPayload = {}, totalChats) => {
		// console.log("extraPayload ===>", message, extraPayload);
		...
		let receiverId =
			chatData.senderId == userData._id
				? chatData.receiverId
				: chatData.senderId;
	...
	render() {
		...
		let { profilePic, userName, deliveredAt, userTypeCode, userStatus = "" } =
			this.state.chatData;
		let { chatData } = this.state;
		...
		return (
			<ChatContext.Consumer>
				{/* <HooksHelperComponent></HooksHelperComponent> */}
				{(data) => {
						...
						chatData && (
							<div className="chat-screen position-relative w-100 h-100">
								<ChatHeader
									...
									handleUnmountHeader={this.handleUnmountHeader}
