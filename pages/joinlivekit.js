...
const JoinLiveKitStream = () => {
    ...
    useEffect(() => {
        if (isTokenGenerated) router.push(`/stream/${isTokenGenerated}`);
    }, [isTokenGenerated])


    const handleJoinRoom = async () => {
        const url = 'wss://video-streaming-2vegkyem.livekit.cloud';
        let roomname = "appscripLivekit"
...
export default JoinLiveKitStream